...
Item {
    ...
    // max number of workspaces to show
    ...
    readonly property HyprlandMonitor currentMonitor: Hyprland.monitorFor(root.QsWindow.window?.screen)
    readonly property Toplevel activeWindow: ToplevelManager.activeToplevel

    readonly property int workspaceGroup: Math.floor((currentMonitor?.activeWorkspace?.id - 1) / root.numWorkspaces)
    ...
    property int currentWorkspace: root.currentMonitor?.activeWorkspace?.id

    property int workspaceIndexInGroup: (root.currentWorkspace - 1) % root.numWorkspaces

    property bool showNumbers: false
    property bool showAppIcons: false

    // color of dot for current workspace
    property string currentColor: Config.colors.fg
    // color of text for current workspace
    property string currentColorFg: Config.colors.bg

    // color of dots for inactive non-empty workspaces
    property string inactiveColor: Config.colors.c8
    // color of text for inactive non-empty workspaces
    property string inactiveColorFg: Config.colors.fg

    // color of dots/numbers for empty workspaces (small dots)
    property string emptyColorFg: Config.colors.c8
    ...
    Connections {
        ...
        function onSuperDownChanged() {
            if (!Config?.options.bar.autoHide.showWhenPressingSuper.enable)
                return;
            if (root.GlobalStates.superDown)
    ...
    function updateWorkspaceOccupied() {
        workspaceOccupied = Array.from({
            length: root.numWorkspaces
        }, (_, i) => {
            return Hyprland.workspaces.values.some(ws => ws.id === workspaceGroup * Config.options.bar.workspaces.shown + i + 1);
        });
    }

    function isWorkspaceActive(index) {
        const wsId = workspaceGroup * root.numWorkspaces + index;
        return currentMonitor?.activeWorkspace?.id === wsId;
    }

    function isWorkspaceOccupied(index) {
        const wsId = workspaceGroup * root.numWorkspaces + index;
    ...
    GridLayout {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                radius: (width / 2)
                property var previousOccupied: (workspaceOccupied[index - 1] && !(!activeWindow?.activated && currentMonitor?.activeWorkspace?.id === index))
                property var rightOccupied: (workspaceOccupied[index + 1] && !(!activeWindow?.activated && currentMonitor?.activeWorkspace?.id === index + 2))
                ...
                opacity: (workspaceOccupied[index] && !(!activeWindow?.activated && currentMonitor?.activeWorkspace?.id === index + 1)) ? 1 : 0
        ...
        Repeater {
            ...
            Button {
                ...
                background: Item {
                    ...
                    Text {
                        opacity: (root.showNumbers || isWorkspaceActive(workspaceValue) || (root.showNumbers && (!root.showAppIcons || !workspaceButtonBackground.biggestWindow)) ? 1 : 0)
                        z: 3
                        ...
                        color: (root.currentMonitor?.activeWorkspace?.id == button.workspaceValue) ? root.currentColorFg : (workspaceOccupied[index] ? root.inactiveColorFg : root.emptyColorFg)
                    ...
                    Rectangle { // Dot instead of ws number
                        id: wsDot
                        opacity: (root.showNumbers || isWorkspaceActive(workspaceValue) || (root.showAppIcons && workspaceButtonBackground.biggestWindow)) ? 0 : 1
                        visible: opacity > 0
                        anchors.centerIn: parent
                        width: root.workspaceButtonWidth * 0.16
                        height: width
                        radius: width / 2
                        color: (root.currentMonitor?.activeWorkspace?.id == button.workspaceValue) ? root.currentColorFg : (workspaceOccupied[index] ? root.inactiveColorFg : root.emptyColorFg)
                    ...
                    Item {
                        ...
                        opacity: !root.showAppIcons ? 0 : (workspaceButtonBackground.biggestWindow && !root.showNumbers && root.showAppIcons) ? 1 : workspaceButtonBackground.biggestWindow ? workspaceIconOpacityShrinked : 0
                        visible: opacity > 0
                        IconImage {
                            id: mainAppIcon
